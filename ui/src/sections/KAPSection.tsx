...
export const KAPSection: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const disclosuresResult = await kapAPI.getDisclosures('THYAO', 5);
                setDisclosures(disclosuresResult.data || disclosuresResult);

                const detailsResult = await kapAPI.getDetails('THYAO');
                setCompanyDetails(detailsResult);
            } catch (error) {
